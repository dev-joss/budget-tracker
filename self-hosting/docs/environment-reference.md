...
| `PLAID_CLIENT_ID`, `PLAID_SECRET`                                              | Plaid API credentials. Both are required to enable Plaid.                                                                                                                   |
| `PLAID_ENV`                                                                    | Plaid environment: `sandbox` or `production`.                                                                                                                               |
| `PLAID_COUNTRY_CODES`                                                          | Comma-separated Plaid country codes. Defaults to `US,CA`.                                                                                                                   |
| `PLAID_REDIRECT_URI`                                                           | OAuth return URL, for example `https://budget.example.com/plaid-callback`. Production requires HTTPS.                                                                       |
| `PLAID_WEBHOOK_URL`                                                            | Public Plaid webhook URL, for example `https://budget.example.com/api/v1/bank-data-providers/plaid/webhook`. Production requires HTTPS.                                     |
| `PLAID_TRANSACTIONS_DAYS_REQUESTED`                                            | Initial transaction history, from 90 through 730 days. Defaults to `180`.                                                                                                   |
